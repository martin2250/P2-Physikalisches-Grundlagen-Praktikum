...
\todo{This method of biasing vastly reduces the effects of varying $\beta$ as a cause of temperature drift by holding the base bias at a constant voltage. (what did you actually want to say?)}
\comp{R_E} is used to compensate for temperature effects by implementing a negative feedback \todo{implementing a negative feedback sounds shitty}, albeit sacrificing an amount of amplification, \todo{this sentence is long enough as it is already, what does the rest have to do with RE?} depending on the conductance $S=\frac{I_\text{C,bias}}{U_\text{T}}$, where $U_\text{T}$ denotes the temperature voltage ($\approx\SI{26}{\milli\volt}$).
...
where $U_\text{a}$ denotes the output voltage, $U_\text{e}$ the input voltage and $T$ the temperature.\todo{only when DC coupled}
This means that temperature drift is proportional to the voltage amplification $A$ in a common emitter amplifier.
Therefore, temperature drift can be lowered by decreasing the amplification. \todo{then why not just use a smaller RC?, not just lower amplification!}

...
This method of biasing vastly reduces the effects of varying $\beta$ as a cause of temperature drift by holding the base bias at a constant voltage. \todo{That this method of biasing provides best temperature stability.}
\comp{R_E} is used to compensate for temperature effects by implementing a negative feedback \todo{I know, suggestions?}, albeit sacrificing an amount of amplification, \todo{what do you mean? i described the effects of introducing RE} depending on the conductance $S=\frac{I_\text{C,bias}}{U_\text{T}}$, where $U_\text{T}$ denotes the temperature voltage ($\approx\SI{26}{\milli\volt}$).
...
where $U_\text{a}$ denotes the output voltage, $U_\text{e}$ the input voltage and $T$ the temperature.\todo{''if the input voltage is kept constant...'' see above}
This means that temperature drift is proportional to the voltage amplification $A$ in a common emitter amplifier.
Therefore, temperature drift can be lowered by decreasing the amplification. \todo{yeah, you're right. how should i explain why C is there in the first place? will halt iwie reinbringen, dass der CE wechselstrommäßig kurzschließt und damit keinen einfluss auf die arbeitspunkteinstellung hat}
...
This result may be explained by the ESR (\textbf{E}quivalent \textbf{S}eries \textbf{R}esistance) of \comp{C_E} which poses an residual resistance at high frequencies instead of the capacitor being a short circuit.
...
%see p14 http://whites.sdsmt.edu/classes/ee320/notes/320Lecture23.pdf
%though I don't think we have all component values we need

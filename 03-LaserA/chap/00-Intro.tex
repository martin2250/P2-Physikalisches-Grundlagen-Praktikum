...
By trivial calculations it can be shown that
...
\begin{figure}[tb]
	\begin{subfigure}{.6\textwidth}
		\centering
		\includegraphics[width=0.6\textwidth]{./img/single_path_diff.pdf}
		\caption[Single slit I]{Path difference deviation}
		\label{fig:path_diff_single}
	\end{subfigure}
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=0.5\textwidth]{./img/single_angle_dist.pdf}
...
Considering \autoref{fig:path_diff_single} it is easy to see that	%I did you ignorant fuck :D (read the '=' sign as 'is equal to' and you'll see)
...
\end{equation}
Like this, the beam can be divided into $2n+1$ partial beams, the $2n$ beams canceling each other out again, while the last one remains.
...
Assuming the observed angles are small, using equations \ref{eq:path_diff_min}, \ref{eq:path_diff_max} and \ref{eq:dist_single}, it can be deduced that
...
To deduce the diffraction pattern of a bridge, Babinet's theorem can be made use of, which states that two complementary objects have the same diffraction pattern.
This can be understood considering following thoughts:
Assuming $O$ is the original diffracting object and $\bar{O}$ is its complement meaning that it is opaque where $O$ is transparent and vice versa.
...
Obviously, a bridge of thickness $b$ is the complement of a single-slit of same gap-width, so the diffraction patterns should be the same and the same equations as deviated in \autoref{subsec:single_slit} can be used.
...
Rotationally symmetrical patterns are expected.
...
Noting that lattices are just $n$ subsequent slits placed in intervals of $\frac{1}{g}$, it can be deduced that
\begin{alignat}{3}
...
For all kinds of slits used in this experiment, however, the exact relation, that is $n$ or $\left(n+\frac{1}{2}\right)$, does not matter in a plot order versus distance between features, since only the slope is of interest to determine distance between slits or gap width respectively.
Hence, measuring distances between maxima or minima do not matter, since they only cause an offset but do not change the slope.
